...
async function checkPros() {
    const { data: pros } = await supabase.from('users').select('uuid, nome, tipo, ativo').ilike('tipo', 'profissional');
    const { data: ags } = await supabase.from('agenda').select('id, profissional');
    ...
    const m = 5;
    const counts = {};
    ags.forEach(a => { if (a.profissional) counts[a.profissional] = (counts[a.profissional] || 0) + 1; });
    ...
    console.log('--- ALL PROFESSIONALS SORTED BY rankingScore ---');
    stats.sort((a, b) => {
        if (b.rankingScore !== a.rankingScore) return b.rankingScore - a.rankingScore;
        return b.count - a.count;
    }).forEach(p => {
        console.log(`${p.rankingScore.toFixed(2)} Score | ${p.count} svc | ${p.rating.toFixed(1)} (${p.v} rats) | ${p.nome} (${p.uuid})`);
    });
}
