...
type FilterType = 'todos' | 'aberto' | 'agendados' | 'execucao' | 'finalizados';
...
const ClientOrders: React.FC = () => {
  ...
  const filteredOrders = useMemo(() => {
      return orders.filter(order => {
          ...
          if (activeFilter === 'aberto') return ['pendente', 'analise', 'aguardando_aprovacao', 'aguardando_profissional', 'reprovado'].includes(status);
  ...
  const filterCounts = useMemo(() => {
      return {
          todos: orders.length,
          aberto: orders.filter(o => ['pendente', 'analise', 'aguardando_aprovacao', 'aguardando_profissional', 'reprovado'].includes(o.status.toLowerCase())).length,
  ...
  const handleProposalDecision = async (orderId: number, approved: boolean, reason: string = '') => {
      ...
      try {
          ...
          if (!approved && reason) {
              updatePayload.motivo_recusa = reason;
          }
          
          const { error: updateError } = await supabase.from('chaves').update(updatePayload).eq('id', orderId);
          ...
          else alert("Serviço agendado com sucesso!");
      } catch (e: any) {
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/40 backdrop-blur-md animate-in fade-in duration-300">
              <div className="bg-white w-full max-sm rounded-[2.5rem] shadow-2xl p-6 flex flex-col gap-6">
                  ...
                  <div className="flex flex-col gap-2">
                      <button disabled={processingAction || !rejectionReason.trim()} onClick={() => handleProposalDecision(selectedOrder.id, false, rejectionReason)} className="w-full bg-black text-white py-4 rounded-2xl font-black text-sm shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-30">{processingAction ? <Loader2 className="animate-spin" size={18}/> : 'Confirmar Recusa'}</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
                ...
                <div className="flex border-b border-gray-100 bg-white overflow-x-auto no-scrollbar h-14 shrink-0">
                    ...
                    <button onClick={() => setActiveTab('obs')} className={`flex-1 min-w-[100px] h-full flex flex-col items-center justify-center transition-all relative group`}><span className={`text-xs font-black uppercase tracking-widest leading-none ${activeTab === 'obs' ? 'text-ios-blue' : 'text-gray-400'}`}>Notas</span>{activeTab === 'obs' && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-ios-blue rounded-t-full" />}</button>
                ...
                <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-white no-scrollbar">
                                ...
                                <div className="grid grid-cols-1 gap-6 mt-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm font-bold text-gray-900">{selectedOrder.planejamento?.[0]?.visita ? new Date(selectedOrder.planejamento[0].visita).toLocaleString('pt-BR', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'}) : 'Não agendada'}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-sm font-bold text-gray-900">{selectedOrder.planejamento?.[0]?.execucao ? new Date(selectedOrder.planejamento[0].execucao).toLocaleString('pt-BR', {day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit'}) : 'A definir'}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-4 pt-4 border-t border-gray-100">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-base font-black text-gray-900">{selectedOrder.orcamentos?.[0]?.preco && selectedOrder.status.toLowerCase() !== 'aguardando_profissional' ? `R$ ${selectedOrder.orcamentos[0].preco.toFixed(2)}` : 'Calculando...'}</p>
                                        </div>
                                    </div>
                                </div>

                                {selectedOrder.orcamentos?.[0] && selectedOrder.status === 'aguardando_aprovacao' && (
                                    <div className="mt-8 pt-8 border-t border-gray-200 animate-in fade-in duration-500">
                                        ...
                                        <div className="grid grid-cols-1 gap-3">
                                            <button onClick={() => setPaymentChoice('original')} className={`p-5 rounded-3xl border-2 text-left transition-all relative overflow-hidden ${paymentChoice === 'original' ? 'border-ios-blue bg-blue-50/50 shadow-md ring-4 ring-blue-50' : 'border-gray-200 bg-white'}`}>
                                                <div className="flex justify-between items-start">
                                                    <div className="flex gap-3">
                                                        <div className={`p-2 rounded-xl ${paymentChoice === 'original' ? 'bg-ios-blue text-white' : 'bg-gray-100 text-gray-400'}`}>{selectedOrder.orcamentos[0].tipopagmto === 'PIX' || selectedOrder.orcamentos[0].tipopagmto === 'Dinheiro' ? <Smartphone size={20}/> : <CreditCard size={20}/>}</div>
                                                        <div>
                                                            <p className="text-xs font-black text-gray-900 uppercase">Sua Escolha Inicial</p>
                                                            <h4 className="text-sm font-bold text-gray-700">{selectedOrder.orcamentos[0].tipopagmto}</h4>
                                                            {selectedOrder.orcamentos[0].tipopagmto === 'Cartão de Crédito' && <p className="text-[10px] font-bold text-gray-400">{selectedOrder.orcamentos[0].parcelas}x no cartão</p>}
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                            ...
                                            {selectedOrder.orcamentos[0].tipopagmto_sugerido && (
                                                <button onClick={() => setPaymentChoice('suggested')} className={`p-5 rounded-3xl border-2 text-left transition-all relative overflow-hidden ${paymentChoice === 'suggested' ? 'border-blue-600 bg-blue-50 shadow-md ring-4 ring-blue-50' : 'border-gray-200 bg-white'}`}>
                                                    ...
                                                    <div className="flex justify-between items-start mt-1">
                                                        <div className="flex gap-3">
                                                            <div className={`p-2 rounded-xl ${paymentChoice === 'suggested' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{selectedOrder.orcamentos[0].tipopagmto_sugerido === 'PIX' || selectedOrder.orcamentos[0].tipopagmto_sugerido === 'Dinheiro' ? <Smartphone size={20}/> : <CreditCard size={20}/>}</div>
                                                            <div>
                                                                <p className="text-xs font-black text-gray-900 uppercase">Oferta Alternativa</p>
                                                                <h4 className="text-sm font-bold text-gray-700">{selectedOrder.orcamentos[0].tipopagmto_sugerido}</h4>
                                                                {selectedOrder.orcamentos[0].tipopagmto_sugerido === 'Cartão de Crédito' && <p className="text-[10px] font-bold text-gray-400">{selectedOrder.orcamentos[0].parcelas_sugerido}x no cartão</p>}
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                    ...
                                                    {selectedOrder.orcamentos[0].justificativa_sugerido && selectedOrder.orcamentos[0].tipopagmto_sugerido !== selectedOrder.orcamentos[0].tipopagmto && (
                                                        <div className="mt-4 p-3 bg-blue-100/50 rounded-2xl border border-blue-200">
                                                            ...
                                                            <p className="text-xs font-bold text-blue-900 italic leading-relaxed">"{selectedOrder.orcamentos[0].justificativa_sugerido}"</p>
                                                        </div>
                                                    )}
                                                </button>
                                            )}
                                        ...
                                        {selectedOrder.orcamentos[0].observacaocliente && (
                                            <div className="mt-6 p-5 bg-blue-600 rounded-3xl text-white shadow-xl relative animate-in slide-in-from-left-4">
                                                ...
                                                <p className="text-sm font-medium leading-relaxed italic">"{selectedOrder.orcamentos[0].observacaocliente}"</p>
                    ...
                    {activeTab === 'obs' && (
                        <div className="space-y-4 animate-in fade-in duration-300">
                            ...
                            <div className="w-full bg-yellow-50 border border-yellow-100 rounded-2xl p-5 text-sm font-bold text-gray-900 min-h-[150px] leading-relaxed">{executionData.observacoes || "Nenhuma anotação registrada pelo profissional."}</div>
                        </div>
                    )}
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                            {!selectedOrder.avaliacao ? (
                                ...
                                <div className="bg-green-50 p-8 rounded-[2.5rem] border border-green-100 text-center space-y-4">
                                    ...
                                    <div className="flex justify-center space-x-1">{[1, 2, 3, 4, 5].map((star) => (<Star key={star} size={28} className={star <= (selectedOrder.avaliacao?.nota || 0) ? 'fill-green-500 text-green-500' : 'text-green-200'} />))}</div>
